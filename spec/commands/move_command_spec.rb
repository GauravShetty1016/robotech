...
RSpec.describe Robotech::Commands::MoveCommand do
  it "should return false when the position is invalid" do
    command = Robotech::Commands::MoveCommand.new(command: ["move"], position: nil, ruleset: double({ valid_position?: true }))
    expect(command.perform).to be false
  end

  it "should return false when the command is not move" do
    command = Robotech::Commands::MoveCommand.new(command: ["moving"], position: nil, ruleset: double({ valid_position?: true }))
    expect(command.perform).to be false
  end

  it "should raise an error when the command is nil" do
    expect { Robotech::Commands::MoveCommand.new(command: nil, position: { x: 1, y: 2, facing_index: 1 }, ruleset: double({ valid_position?: true })) }.to raise_error(ArgumentError)
  end
